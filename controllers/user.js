...
exports.create = async (req, res) => {
    try {
        const user = await User.findOne({ email: req.body.email });
        if (user){
            res.render('create-user', { errors: { email: { message: 'account with this email already exists' } } })
            return;
        }
        const userToCreate = new User({ email: req.body.email, password: req.body.password });
